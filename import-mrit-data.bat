...
REM Create transformation script
...
TRUNCATE student_data, student_variables, faculty, course_offering, timetable CASCADE;
...
WHERE usn IS NOT NULL AND active = true
ON CONFLICT (usn) DO NOTHING;

-- Import Student Variables
...
LEFT JOIN section s ON s.name = sd.sec_name
WHERE sd.usn IS NOT NULL AND sd.active = true
ON CONFLICT (usn) DO NOTHING;

-- Import Faculty
INSERT INTO faculty (
    employee_id, faculty_name, short_name, phone, email,
    department_id, designation_id, active, gender_id
)
SELECT 
    employee_id, 
    employee_name, 
    short_name, 
    phone, 
    COALESCE(email_org, email_personal) as email,
    dept_id, 
    designation_id, 
    active, 
    gender_id
FROM public.employee_data
WHERE active = true AND dept_id IS NOT NULL
ON CONFLICT (employee_id) DO NOTHING;

-- Import Course Offerings from timetable
INSERT INTO course_offering (
    course_id, faculty_id, section_id, academic_year_id, active
)
SELECT DISTINCT
    c.id as course_id,
    f.id as faculty_id,
...
JOIN course c ON c.course_code = tt.course_code
JOIN faculty f ON f.employee_id = tt.faculty_id
JOIN section s ON s.id = tt.section_id
WHERE tt.course_code IS NOT NULL 
  AND tt.faculty_id IS NOT NULL
ON CONFLICT DO NOTHING;

-- Import Timetable
INSERT INTO timetable (
    course_offering_id, time_slot_id, day_of_week, room_number, active
)
SELECT DISTINCT
    co.id as course_offering_id,
    tt.class_id as time_slot_id,
    tt.weekday_id as day_of_week,
    COALESCE(rn.room_number, 'TBA') as room_number,
    true as active
FROM time_table.tt_2425 tt
JOIN course c ON c.course_code = tt.course_code
JOIN faculty f ON f.employee_id = tt.faculty_id
...
JOIN course_offering co ON co.course_id = c.id 
    AND co.faculty_id = f.id 
    AND co.section_id = s.id
    AND co.academic_year_id = tt.aca_yr_id
LEFT JOIN infra.room_numbers rn ON rn.id = tt.room_number_id
WHERE tt.course_code IS NOT NULL
ON CONFLICT DO NOTHING;
